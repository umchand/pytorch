...
from .. import config
from .common import DeferredLine, IndentedBuffer, Kernel, SizeArg, PythonPrinter
...
def _default_custom_combo_kernel_horizontal_partition(nodes, triton_scheduling, kernel_map, node_info_map):
    ...
    for i in range(2, max_dims+1):
        ...
        reduction = [n for n in group_per_dim if kernel_map[n].inside_reduction and not (kernel_map[n].persistent_reduction and kernel_map[n].no_x_dim)]
        ...
        if long_reduction:
            logging.warning(f"ComboKernels: {len(long_reduction)} long reduction nodes are separated")
        large_pointwise = [n for n in not_reduction if not kernel_map[n].inside_reduction and len(kernel_map[n].numels) == 2 and V.graph.sizevars.size_hint(kernel_map[n].numels[0]) > 512e5]
        if large_pointwise:
            # TODO benchmark the performance when large pointwise nodes combining with others
            logging.warning(f"ComboKernels: {len(large_pointwise)} large pointwise nodes are separated")
...
_custom_combo_kernel_horizontal_partition_algorithm = _default_custom_combo_kernel_horizontal_partition
...
class ComboKernel(Kernel):
    ...
    def _base_horizontal_partition(subkernel_nodes, triton_scheduling, node_info_map):
        ...
        ndim_to_partition_state: Dict[Integer, PartitionState] = defaultdict(
    ...
    @staticmethod
    def horizontal_partition(nodes, triton_scheduling, kernel_map, node_info_map, custom_algorithm=False):
        ...
        for raw_partition in raw_partitions:
            all_partitions.extend(ComboKernel._base_horizontal_partition(raw_partition, triton_scheduling, node_info_map))
        return all_partitions
    ...
    def codegen_static_numels_sub_kernel(self, code, sub_kernel, num):
        ...
        for tree in sub_kernel.range_trees:
            ...
            # code.writeline(f"{tree.prefix.upper()}BLOCK: tl.constexpr = {tree.prefix.upper()}BLOCK_{num}")
    ...
    def select_heuristics(self, sub_kernel) -> Tuple[str, List[int]]:
        size_hints = [
            next_power_of_2(V.graph.sizevars.size_hint(numel)) for numel in sub_kernel.numels
        ]
    ...
    def select_combo_heuristics(self, heuristics_list, size_hints_list):
        if "reduction" in heuristics_list:
            # i = heuristics_list.index("reduction")
            i, _ = max(enumerate(size_hints_list), key=lambda x: x[1][0] if heuristics_list[x[0]] == "reduction" else 0)
            return heuristics_list[i], size_hints_list[i], self.sub_kernels[i]
        elif "pointwise" in heuristics_list:
            # i = heuristics_list.index("pointwise")
            i, _ = max(enumerate(size_hints_list), key=lambda x: x[1][0] if heuristics_list[x[0]] == "pointwise" else 0)
            ...
            num_persistent_reduction = len([e for e in heuristics_list if e == "persistent_reduction"])
            assert num_reduction == 0, "combining pointwise and reduction are not supported yet."
            heuristics = "pointwise_with_reduction" if num_persistent_reduction > 0 else "pointwise"
    ...
    def get_mutated_args_sub_kernels(self) -> Set[str]:
        mutated_args = set()
        for sub_kernel in self.sub_kernels:
            for mutation in sub_kernel.mutations:
                ...
                if (
                    ...
                    mutated_args.add(sub_kernel.args.inplace_buffers[mutation].inner_name)
    ...
    def jit_line(self, heuristics, size_hints, selected_kernel, pointwise_with_reduce=False):
    ...
    def codegen_kernel(self, name=None):
        ...
        heuristics, size_hints, selected_kernel = self.select_combo_heuristics(heuristics_list, size_hints_list)
        pointwise_with_reduction, heuristics = (True, "pointwise") if heuristics == "pointwise_with_reduction" else (False, heuristics)
        code = IndentedBuffer()
        ...
        with code.indent():
            ...
            for num, sub_kernel in enumerate(self.sub_kernels):
                ...
                with code.indent():
                    uniquify = self.codegen_static_numels_sub_kernel(code, sub_kernel, num)
                    sub_kernel.codegen_body()
                    uniquified_body = self.uniquify_block_sizes(sub_kernel.body, num, uniquify)
    ...
    def uniquify_block_sizes(self, code: IndentedBuffer, num_kernel, uniquify: List[str]) -> IndentedBuffer:
        if not uniquify:
            return code
        ...
        for line in code._lines:
            if isinstance(line, str) and (blocks:=[e for e in uniquify if e in line]):
                modified_line = line
                for block in blocks:
                    modified_line = modified_line.replace(block, f"{block}_{num_kernel}")
                modified.writeline(modified_line)
            elif isinstance(line, DeferredLine) and (blocks:=[e for e in uniquify if e in line.line]):
    ...
    def grid(self, sub_kernel_numels):
        ...
        xnumel = None if any(e is None for e in xnumel) else max(xnumel) # sum(xnumel)
        ynumel = None if any(e is None for e in ynumel) else max(ynumel)
        znumel = None if any(e is None for e in znumel) else max(znumel)

        numels = (xnumel,) if not ynumel else (ynumel, xnumel) if not znumel else (znumel, ynumel, xnumel)
        return numels
