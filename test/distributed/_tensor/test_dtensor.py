...
class DTensorMeshTest(DTensorTestBase):
    ...
    def test_auto_implicit_replication(self):
        ...
        # automatically turn tensor to DTensor replicate when ndim = 0
        ndim_0_tensor = torch.tensor(1, device=self.device_type)
        self.assertEqual(
            (sharded_dtensor + ndim_0_tensor).to_local(), local_tensor + ndim_0_tensor
        )

        # automatically turn tensor to DTensor replicate when numel = 0
